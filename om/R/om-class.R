...
#' @description Operating model class definition.
#' @slot .Data numbers generated by the model run as the sum of 1+ age classes. 
...
#' @slot diagnostics list containing realised model outputs \code{capture}, \code{depletion} and \code{harvest_rate}. 
#' @slot values data frame of values sampled from the life-history input distributions during execution of the model.
#' @slot seeds random number seeds generated when the \code{om} is initialised and used throughout. 
...
setMethod("initialize", "om", function(.Object, ages, time, harvest_function, samples = 1, shape = 1, phi = 1) {
    ...
    # (carrying capacity)
    .Object@pars$K <- distribution(values = 1, name = "K1+")
